...
### Grouping incident types (not necessary for analysis)
```{r incidents}
incidents_simple<-select(incidents_data, dispatch_date, text_general_code)
incident_counts<-incidents_simple %>%
  group_by(dispatch_date, text_general_code) %>%
  summarise(count=n())

incidents_wide<-dcast(incident_counts, dispatch_date~text_general_code, value.var="count", fill=0)

##Incident categories
violent_incidents<-c("Aggravated Assault Firearm", "Aggravated Assault No Firearm", "Homicide - Criminal", "Homicide - Criminal ", "Homicide - Justifiable ", "Homicide - Gross Negligence", "Other Assaults", "Rape", "Robbery Firearm", "Robbery No Firearm")

property_incidents<-c("Burglary Non-Residential", "Burglary Residential", "Motor Vehicle Theft", "Theft from Vehicle", "Thefts", "Vandalism/Criminal Mischief", "Embezzlement", "Forgery and Counterfeiting", "Recovered Stolen Motor Vehicle", "Arson", "Fraud", "Receiving Stolen Property")

drugs_incidents<-c("DRIVING UNDER THE INFLUENCE", "Narcotic / Drug Law Violations",  "Liquor Law Violations", "Public Drunkenness")

firearms_incidents<-c("Weapon Violations")

other_incidents<-c("Vagrancy/Loitering", "Offenses Against Family and Children", "Gambling Violations", "All Other Offenses", "Disorderly Conduct", "Other Sex Offenses (Not Commercialized)","Prostitution and Commercialized Vice")

#codes2<-c(violent_incidents, property_incidents, drugs_incidents, firearms_incidents, other_incidents)
#setdiff(codes, codes2)
